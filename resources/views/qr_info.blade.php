                ...
                <div class="question_inner">
                    ...
                    <div class="question_body flex is-between">
                        <div class="question_left">
                            ...
                            <p class="question_left_bgname">該当のボードゲーム <a class="boardgame_tag"
                                    href="{{route('boardgame',['id' => $questions->boardgame_id])}}">{{$questions->boardgame->name}}</a>
                            </p>
                        </div>
                        <div class="question_right">
                            <p><img src="../storage/{{$questions->user->profile_image}}" alt="">
                                <a
                                    href="{{ Route('show_user',['id' => $questions->user_id]) }}">{{ $questions->user->screen_name}}</a>
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-8">
            ...
            <div class="answerWrap">
                ...
                <form action=" {{ Route('bestanswer') }}">
                    ...
                    <button type="submit" class="btn p-0 border-0 text-secondary"><i
                            class="material-icons">done</i>この回答をベストアンサーにする</button>
                </form>
                @endif
                @endif
                <div class="answer_text">
                    ...
                    <div class="d-flex justify-content-end">
                        <p class="answer_text_time" class="mr-1">
                            投稿<span>{{ $answer->created_at->format('Y/m/d H:i') }}</span></p>
                        <p class="user_content"><img src="../storage/{{ $answer->user->profile_image }}" alt=""></p>
                        <p><a href="">{{ $answer->user->screen_name }}</a></p>
                    </div>
                    {{-- コメントを表示 --}}
                    <div class="show_comment_wrap">
                        @foreach(($answer->comments) as $comment)
                        <div class="flex show_comment_body">
                            <p>
                                <img src="../storage/{{$comment->user->profile_image}}" alt="">
                            </p>
                            <div>
                                <p class="show_comment_title">{{$comment->user->screen_name}} <span>{{$comment->created_at->format('Y/m/d H:i')}}</span>
                                </p>
                                <p>{{$comment->text}}</p>
                            </div>
                        </div>
                    ...
                    <div class="comment_wrap">
                        ...
                        <form action="{{Route('comment_insert')}}" method="post">
                            @csrf
                            <textarea class="commnet_textarea" name="text" id="" cols="40" rows="3" required></textarea>
                            <input type="hidden" name="answer_id" value="{{ $answer->id }}">
                            <input type="hidden" name="user_id" value="{{ Auth::id() }}">
                            <div class="comment_btn"><input id='modal_btn' type="submit" value="コメントする" required></div>
                        </form>
                    </div>
                    @endif
                    @else
                ...
                <div class="flex post_answer_wrap">
                    ...
                    <div>
                        ...
                        <form action="{{Route('show',['id' => $questions->id])}}" method="post">
                            ...
                            <input type="hidden" name="user_id" value="{{ Auth::id()}}">
                            <div class="question_btn"><input id='modal_btn' type="submit" value="回答する" required></div>
                        </form>
                    </div>
                </div>
                @endif
